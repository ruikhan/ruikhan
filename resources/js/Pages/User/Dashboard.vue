...
const startCinematicSequence = () => {
    ...
    setTimeout(() => { 
        if (!skipIntro.value) {
            ...
            playSound(1047, 0.3); // C note (high)
        }
    }, 19000);
    ...
    setTimeout(() => {
        if (!skipIntro.value) {
            introPhase.value = 'fadeout';
            playSound(1319, 0.4); // E note (high)
            setTimeout(() => { 
                showIntro.value = false;
                triggerEntranceAnimation();
            }, 1000);
...
onMounted(() => {
    ...
    // Check session storage - only show intro once per session
            ...
            <div v-if="showIntro" class="cinematic-overlay">
                ...
                <div class="cinematic-bg">
                    ...
                    <div class="cinematic-particles">
                        <!-- Floating dots -->
                        <div v-for="i in 60" :key="`dot-${i}`" class="particle-dot"
                             :style="{
                        ...
                        <!-- Stars -->
                        <div v-for="i in 30" :key="`star-${i}`" class="particle-star"
                             :style="{
                                 left: `${Math.random() * 100}%`,
                                 top: `${Math.random() * 100}%`,
                        ...
                        <!-- Sparkles -->
                        <div v-for="i in 20" :key="`sparkle-${i}`" class="particle-sparkle"
                             :style="{
                                 left: `${Math.random() * 100}%`,
                                 top: `${Math.random() * 100}%`,
                                ...
                                <div :key="currentFeatureIndex" class="feature-showcase">
                                    ...
                                    <div class="feature-progress">
                                        <div v-for="(f, i) in cinematicFeatures" :key="i"
                                             ...
                                             :style="i === currentFeatureIndex ? `background: linear-gradient(to right, var(--tw-gradient-stops))` : ''"
                                             :data-gradient="currentFeature.gradient">
                                        </div>
                                    </div>
                                </div>
                            </TransitionGroup>
...
* { font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Segoe UI', sans-serif; -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; letter-spacing: -0.01em; }
...
.mesh-blob { @apply absolute rounded-full blur-[120px]; animation: mesh-float 20s ease-in-out infinite; }
.mesh-blob-1 { @apply w-[600px] h-[600px] top-[10%] left-[10%]; background: radial-gradient(circle, rgba(59, 130, 246, 0.3), transparent); }
.mesh-blob-2 { @apply w-[500px] h-[500px] bottom-[15%] right-[15%]; background: radial-gradient(circle, rgba(147, 51, 234, 0.3), transparent); animation-delay: 5s; }
.mesh-blob-3 { @apply w-[550px] h-[550px] top-[40%] right-[30%]; background: radial-gradient(circle, rgba(236, 72, 153, 0.2), transparent); animation-delay: 10s; }
...
.cinematic-particles { @apply absolute inset-0; }
.particle-dot { @apply absolute w-1 h-1 bg-white rounded-full; animation: particle-float 8s ease-in-out infinite; }
@keyframes particle-float { 0%, 100% { transform: translate(0, 0); opacity: 0.2; } 50% { transform: translate(80px, -100px); opacity: 0.8; } }

.particle-star { @apply absolute text-xs; animation: star-twinkle 2s ease-in-out infinite; }
@keyframes star-twinkle { 0%, 100% { opacity: 0.3; transform: scale(0.8); } 50% { opacity: 1; transform: scale(1.2); } }

.particle-sparkle { @apply absolute text-sm; animation: sparkle-spin 3s ease-in-out infinite; }
@keyframes sparkle-spin { 0%, 100% { opacity: 0.4; transform: rotate(0deg) scale(0.8); } 50% { opacity: 1; transform: rotate(180deg) scale(1.3); } }

.cinematic-grid { @apply absolute inset-0 opacity-[0.02]; background-image: linear-gradient(rgba(255,255,255,0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.1) 1px, transparent 1px); background-size: 50px 50px; }

.skip-btn { @apply absolute top-8 right-8 z-50 px-6 py-3 bg-white/5 backdrop-blur-xl border border-white/10 rounded-full text-white text-sm font-semibold hover:bg-white/10 transition-all duration-300 flex items-center gap-2 hover:scale-105 active:scale-95; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.3); }
...
.logo-glow { @apply absolute inset-0 bg-blue-500/30 blur-[100px] rounded-full; animation: logo-pulse 2s ease-in-out infinite; }
@keyframes logo-pulse { 0%, 100% { opacity: 0.3; transform: scale(1); } 50% { opacity: 0.6; transform: scale(1.2); } }
.logo-icon { @apply relative text-9xl; filter: drop-shadow(0 0 40px rgba(59, 130, 246, 0.8)); animation: logo-spin 20s linear infinite; }
@keyframes logo-spin { from { transform: rotateY(0deg); } to { transform: rotateY(360deg); } }
.logo-ripple { @apply absolute inset-0 border-4 border-blue-500/20 rounded-full; animation: ripple 2s ease-out infinite; }
@keyframes ripple { 0% { transform: scale(0.8); opacity: 1; } 100% { transform: scale(2); opacity: 0; } }
.logo-text { @apply text-6xl font-black text-white mb-4 tracking-wider; text-shadow: 0 0 60px rgba(59, 130, 246, 0.5); }
.logo-subtitle { @apply text-xl text-gray-400 font-light tracking-widest uppercase; }

/* Welcome Phase */
.welcome-badge { @apply inline-flex items-center gap-3 px-6 py-3 rounded-full mb-10 bg-blue-500/10 border border-blue-500/30 backdrop-blur-xl; }
...
.pulse-dot { @apply relative rounded-full h-3 w-3 bg-blue-500; box-shadow: 0 0 20px rgba(59, 130, 246, 1); }
.welcome-badge-text { @apply text-sm font-bold text-blue-300 uppercase tracking-[0.3em]; }
.welcome-greeting { @apply text-6xl md:text-8xl font-thin text-white mb-6 tracking-tight; animation: text-shimmer 3s ease-in-out infinite; }
@keyframes text-shimmer { 0%, 100% { opacity: 0.8; } 50% { opacity: 1; } }
.welcome-name { @apply text-7xl md:text-9xl font-black text-transparent bg-clip-text mb-8; background-image: linear-gradient(135deg, #3b82f6, #8b5cf6, #ec4899, #3b82f6); background-size: 300% auto; animation: gradient-flow 4s ease-in-out infinite; }
@keyframes gradient-flow { 0%, 100% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } }
.welcome-subtitle { @apply text-2xl text-gray-400 font-light; animation: fade-in-up 1s ease-out 0.5s both; }
@keyframes fade-in-up { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }

/* Features Phase */
.feature-showcase { @apply space-y-10; }
.feature-icon-wrapper { @apply relative inline-block mb-10; }
.feature-glow { @apply absolute inset-0 blur-[120px] opacity-60 rounded-full; animation: feature-glow-pulse 3s ease-in-out infinite; }
@keyframes feature-glow-pulse { 0%, 100% { opacity: 0.4; transform: scale(1); } 50% { opacity: 0.7; transform: scale(1.3); } }
.feature-particles { @apply absolute inset-0; }
.fp { @apply absolute text-4xl; animation: fp-orbit 4s ease-in-out infinite; }
...
@keyframes fp-orbit { 0%, 100% { transform: scale(0.5) translateY(0); opacity: 0.3; } 50% { transform: scale(1.2) translateY(-20px); opacity: 0.9; } }
.feature-icon { @apply relative text-9xl; filter: drop-shadow(0 0 60px rgba(255, 255, 255, 0.5)); animation: feature-float 4s ease-in-out infinite; }
@keyframes feature-float { 0%, 100% { transform: translateY(0) rotate(0deg); } 50% { transform: translateY(-30px) rotate(5deg); } }
...
.progress-bar { @apply h-1 rounded-full bg-white/20 transition-all duration-500; width: 60px; }
.progress-bar.active { width: 120px; }

/* Stats Phase */
.stats-title { @apply text-5xl md:text-6xl font-black text-white mb-12; }
.stats-showcase { @apply grid grid-cols-3 gap-8 mb-12; }
.stat-item { @apply relative overflow-hidden rounded-3xl p-10 border border-white/10; background: linear-gradient(135deg, rgba(255,255,255,0.05), rgba(255,255,255,0.02)); backdrop-filter: blur(20px); animation: stat-appear 0.8s cubic-bezier(0.34, 1.56, 0.64, 1) both; }
@keyframes stat-appear { from { opacity: 0; transform: scale(0.8) translateY(50px); } to { opacity: 1; transform: scale(1) translateY(0); } }
...
/* Ready Phase */
.ready-icon { @apply text-9xl mb-8; animation: ready-bounce 0.6s cubic-bezier(0.34, 1.56, 0.64, 1); }
@keyframes ready-bounce { 0% { transform: scale(0); } 50% { transform: scale(1.2); } 100% { transform: scale(1); } }
.ready-title { @apply text-6xl font-black text-white mb-4; }
.ready-subtitle { @apply text-2xl text-gray-400 font-light; }

/* Transitions */
.intro-fade-enter-active, .intro-fade-leave-active { transition: opacity 1s ease; }
.intro-fade-enter-from, .intro-fade-leave-to { opacity: 0; }
.phase-fade-enter-active { transition: all 1s cubic-bezier(0.16, 1, 0.3, 1); }
.phase-fade-leave-active { transition: all 0.8s cubic-bezier(0.16, 1, 0.3, 1); }
.phase-fade-enter-from { opacity: 0; transform: scale(0.8); }
.phase-fade-leave-to { opacity: 0; transform: scale(1.2); }
.feature-slide-enter-active { transition: all 1s cubic-bezier(0.16, 1, 0.3, 1); }
.feature-slide-leave-active { transition: all 0.8s cubic-bezier(0.16, 1, 0.3, 1); }
...
/* Toast */
.toast-notification { @apply fixed top-20 right-4 z-[60] w-[90vw] sm:w-96 rounded-2xl shadow-2xl overflow-hidden flex items-center gap-3; background: linear-gradient(135deg, rgba(16, 185, 129, 1), rgba(5, 150, 105, 1)); backdrop-filter: blur(20px); border: 1px solid rgba(255, 255, 255, 0.2); }
...
.toast-close-btn { @apply flex-shrink-0 p-2 hover:bg-white/20 rounded-lg transition-colors active:scale-90; }
.toast-close-icon { @apply w-5 h-5 text-white; }
@keyframes toast-bounce { 0% { opacity: 0; transform: translateX(100%) scale(0.8); } 60% { opacity: 1; transform: translateX(-10px) scale(1.02); } 100% { opacity: 1; transform: translateX(0) scale(1); } }
.toast-bounce-enter-active { animation: toast-bounce 0.6s cubic-bezier(0.34, 1.56, 0.64, 1); }
.toast-bounce-leave-active { transition: all 0.3s cubic-bezier(0.4, 0, 1, 1); }
.toast-bounce-leave-to { opacity: 0; transform: translateX(100%) scale(0.8); }

/* MOBILE & DESKTOP LAYOUTS - Keep all your existing styles from document #3 */
/* I'm including the critical base styles here, but you should merge with your full stylesheet */
...
.time-widget-container { @apply relative overflow-hidden rounded-[2rem] p-5 border border-white/[0.10]; background: linear-gradient(135deg, rgba(28, 28, 30, 1), rgba(44, 44, 46, 1)); box-shadow: 0 20px 60px rgba(0, 0, 0, 0.8); }
.time-widget-gradient { @apply absolute inset-0; background: linear-gradient(135deg, rgba(59, 130, 246, 0.08), rgba(147, 51, 234, 0.08), rgba(236, 72, 153, 0.08)); animation: gradient-rotate 20s ease-in-out infinite; }
@keyframes gradient-rotate { 0%, 100% { transform: rotate(0deg) scale(1); } 50% { transform: rotate(180deg) scale(1.1); } }
.time-widget-glow { @apply absolute top-0 right-0 w-32 h-32 rounded-full blur-3xl; background: radial-gradient(circle, rgba(59, 130, 246, 0.2), transparent); }
...
.time-value { @apply text-4xl font-bold text-white tracking-tight drop-shadow-xl; }
.time-period { @apply text-lg font-semibold text-gray-400; }
.date-badge-wrapper { @apply flex flex-col items-end; }
.date-badge { @apply relative px-4 py-2 rounded-2xl border border-white/[0.12] shadow-lg overflow-hidden transition-transform duration-300; background: rgba(255, 255, 255, 0.10); backdrop-filter: blur(20px); }
.date-badge:hover { @apply scale-105; }
.date-badge-shine { @apply absolute inset-0 bg-gradient-to-br from-white/10 via-transparent to-transparent opacity-0 transition-opacity duration-500; }
...
.stat-card { @apply relative overflow-hidden rounded-[1.75rem] p-5 shadow-xl transition-all duration-500 border border-white/[0.10] opacity-0; background: linear-gradient(135deg, rgba(28, 28, 30, 1), rgba(44, 44, 46, 1)); }
.stat-card:hover { @apply shadow-2xl scale-105 -translate-y-1; }
.stat-card-glow { @apply absolute -top-10 -right-10 w-24 h-24 opacity-25 blur-2xl rounded-full transition-all duration-700; }
.stat-card:hover .stat-card-glow { @apply opacity-40 scale-125; }
.stat-card-shine { @apply absolute inset-0 bg-gradient-to-br from-white/[0.05] via-transparent to-transparent opacity-0 transition-opacity duration-500; }
.stat-card:hover .stat-card-shine { @apply opacity-100; }
.stat-icon-wrapper { @apply relative mb-3; }
.stat-icon-badge { @apply w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg transition-all duration-500; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.3); }
.stat-card:hover .stat-icon-badge { @apply scale-110 rotate-6; }
.stat-icon { @apply text-2xl filter drop-shadow-xl; }
.stat-content { @apply relative; }
.stat-value { @apply text-3xl font-bold text-white mb-0.5 tracking-tight transition-transform duration-300; }
...
.stat-highlight { @apply absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-white/[0.08] to-transparent transition-all duration-500; }
.stat-card:hover .stat-highlight { @apply via-white/[0.15]; }

/* Search */
.search-wrapper { @apply mb-6 opacity-0; }
.search-container { @apply relative; }
.search-glow { @apply absolute -inset-0.5 rounded-[2rem] opacity-0 blur-2xl transition-all duration-700; background: linear-gradient(to right, rgba(59, 130, 246, 1), rgba(147, 51, 234, 1), rgba(236, 72, 153, 1)); }
.search-container:focus-within .search-glow { @apply opacity-25; }
.search-bar { @apply relative bg-[#1c1c1e] rounded-[2rem] border border-white/[0.10] shadow-xl overflow-hidden transition-all duration-500; }
.search-bar:focus-within { @apply border-white/[0.15]; }
.search-highlight { @apply absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/[0.12] to-transparent; }
.search-icon-wrapper { @apply absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center; }
.search-icon { @apply w-5 h-5 text-gray-400 transition-all duration-500; }
.search-bar:focus-within .search-icon { @apply text-blue-400 scale-110; }
.search-input { @apply w-full px-5 py-4 pl-14 bg-transparent text-white placeholder-gray-500 text-[15px] focus:outline-none; }
.search-voice-btn { @apply absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center rounded-xl transition-all duration-300 active:scale-90; background: rgba(255, 255, 255, 0.08); }
.search-voice-btn:hover { background: rgba(255, 255, 255, 0.15); }
...
.apps-view-all { @apply text-xs font-semibold text-blue-400 hover:text-blue-300 active:scale-95 transition-all duration-300; }
.apps-view-all:hover { @apply translate-x-1; }
.apps-grid { @apply grid grid-cols-4 gap-4; }
.app-item { @apply flex flex-col items-center gap-2 rounded-3xl p-2 active:scale-90 transition-all duration-500 opacity-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500; }
...
.app-glow { @apply absolute -inset-1 rounded-[1.4rem] opacity-0 blur-lg transition-all duration-500; }
.app-item:hover .app-glow { @apply opacity-70; }
.app-icon-wrapper { @apply relative w-full h-full rounded-[1.3rem] shadow-xl flex items-center justify-center transition-all duration-500 overflow-hidden; }
.app-item:hover .app-icon-wrapper { @apply scale-110; }
.app-icon-shine { @apply absolute inset-[1px] rounded-[1.2rem] opacity-90; background: linear-gradient(to bottom, rgba(255, 255, 255, 0.3), rgba(255, 255, 255, 0.15), transparent); }
.app-icon-reflection { @apply absolute inset-0 rounded-[1.3rem]; background: linear-gradient(to bottom, rgba(255, 255, 255, 0.25), transparent, transparent); }
.app-icon-shine-rotate { @apply absolute inset-0 rounded-[1.3rem] -translate-x-full transition-transform duration-1000; background: linear-gradient(to right, transparent, rgba(255, 255, 255, 0.2), transparent); }
.app-item:hover .app-icon-shine-rotate { @apply translate-x-full; }
.app-icon-emoji { @apply relative text-[32px] filter drop-shadow-2xl transition-transform duration-300; }
.app-item:hover .app-icon-emoji { @apply scale-110; }
.app-name { @apply text-[11px] font-semibold text-gray-300 text-center leading-tight w-full px-1; }
...
.featured-dot-1 { @apply bg-green-400; box-shadow: 0 0 8px rgba(74, 222, 128, 0.5); }
.featured-dot-2 { @apply bg-blue-400; box-shadow: 0 0 8px rgba(96, 165, 250, 0.5); animation-delay: 0.2s; }
.featured-dot-3 { @apply bg-purple-400; box-shadow: 0 0 8px rgba(192, 132, 252, 0.5); animation-delay: 0.4s; }
.featured-cards-grid { @apply grid grid-cols-2 gap-3; }
.featured-card { @apply relative overflow-hidden rounded-[1.75rem] p-4 border border-white/[0.10] shadow-xl transition-all duration-500 active:scale-[0.97] opacity-0; background: linear-gradient(135deg, rgba(28, 28, 30, 1), rgba(44, 44, 46, 1)); }
.featured-card:hover { @apply shadow-2xl; }
.featured-card-glow { @apply absolute -top-10 -right-10 w-24 h-24 opacity-25 blur-2xl rounded-full transition-all duration-700; }
.featured-card-glow-green { background: radial-gradient(circle, rgba(34, 197, 94, 1), rgba(16, 185, 129, 1)); }
.featured-card-glow-red { background: radial-gradient(circle, rgba(239, 68, 68, 1), rgba(236, 72, 153, 1)); }
.featured-card:hover .featured-card-glow { @apply opacity-40 scale-125; }
.featured-card-shine { @apply absolute inset-0 bg-gradient-to-br from-white/[0.05] via-transparent to-transparent opacity-0 transition-opacity duration-500; }
.featured-card:hover .featured-card-shine { @apply opacity-100; }
.featured-pulse-indicator { @apply absolute top-3 right-3 z-10; }
.featured-pulse-dot { @apply w-2 h-2 bg-red-400 rounded-full animate-pulse shadow-lg; box-shadow: 0 0 8px rgba(248, 113, 113, 0.7); }
.featured-pulse-ring { @apply absolute inset-0 bg-red-400 rounded-full animate-ping; }
.featured-card-icon { @apply mb-3 w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg transition-all duration-500; }
.featured-card-icon-green { background: linear-gradient(135deg, rgba(34, 197, 94, 1), rgba(16, 185, 129, 1)); }
.featured-card-icon-red { background: linear-gradient(135deg, rgba(239, 68, 68, 1), rgba(236, 72, 153, 1)); }
.featured-card:hover .featured-card-icon { @apply scale-110 rotate-6; }
...
.featured-card-arrow { @apply absolute bottom-4 right-4 w-6 h-6 rounded-full flex items-center justify-center transition-all duration-300 z-10; background: rgba(255, 255, 255, 0.10); }
.featured-card:hover .featured-card-arrow { background: rgba(255, 255, 255, 0.15); @apply translate-x-1; }
.featured-arrow-icon { @apply w-3 h-3 text-gray-400; }

/* Map */
.map-preview-card { @apply relative overflow-hidden rounded-[1.75rem] border border-white/[0.10] shadow-xl opacity-0; background: linear-gradient(135deg, rgba(28, 28, 30, 1), rgba(44, 44, 46, 1)); }
.map-preview-container { @apply relative h-52 overflow-hidden; }
.map-badge { @apply absolute top-4 left-4 z-10 flex items-center gap-2 px-3 py-2 rounded-full border border-white/[0.10] shadow-xl; background: rgba(0, 0, 0, 0.7); backdrop-filter: blur(40px); }
.map-badge-pulse { @apply relative flex h-2 w-2; }
.map-badge-ping { @apply animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75; }
.map-badge-dot { @apply relative inline-flex rounded-full h-2 w-2 bg-green-400 shadow-lg; box-shadow: 0 0 8px rgba(74, 222, 128, 0.5); }
.map-badge-text { @apply text-xs font-bold text-white; }
.map-preview-wrapper { @apply w-full h-full; }
.map-overlay { @apply absolute inset-0 bg-gradient-to-t from-[#1c1c1e] via-transparent to-transparent pointer-events-none; }
.map-info-bar { @apply absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 via-black/50 to-transparent; backdrop-filter: blur(20px); }
...
.map-info-arrow { @apply w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300; background: rgba(255, 255, 255, 0.15); }
.map-preview-container:hover .map-info-arrow { background: rgba(255, 255, 255, 0.25); @apply translate-x-1; }
...
.emergency-glow { @apply absolute inset-0 bg-red-500 rounded-full blur-2xl opacity-60 transition-opacity duration-500; }
.floating-emergency-btn:hover .emergency-glow { @apply opacity-80; }
.emergency-btn { @apply relative w-16 h-16 rounded-full flex items-center justify-center active:scale-90 transition-all duration-300 border-4 border-black/30 overflow-hidden; background: linear-gradient(135deg, rgba(239, 68, 68, 1), rgba(220, 38, 38, 1), rgba(185, 28, 28, 1)); box-shadow: 0 20px 60px rgba(239, 68, 68, 0.7); }
.floating-emergency-btn:hover .emergency-btn { @apply scale-110; }
.emergency-btn-shine { @apply absolute inset-[2px] rounded-full; background: linear-gradient(to bottom, rgba(255, 255, 255, 0.40), rgba(255, 255, 255, 0.20), transparent); }
.emergency-btn-shine-rotate { @apply absolute inset-0 rounded-full -translate-x-full transition-transform duration-1000; background: linear-gradient(to right, transparent, rgba(255, 255, 255, 0.30), transparent); }
...
.emergency-label { @apply absolute -bottom-7 left-1/2 -translate-x-1/2 whitespace-nowrap opacity-0 transition-opacity duration-300; }
.floating-emergency-btn:hover .emergency-label { @apply opacity-100; }
.emergency-label-content { @apply px-3 py-1 rounded-full border border-white/20 shadow-xl; background: rgba(0, 0, 0, 0.8); backdrop-filter: blur(20px); }
.emergency-label-text { @apply text-[10px] font-bold text-white; }

/* Desktop */
.desktop-header { @apply mb-8 opacity-0; }
.desktop-header-content { @apply flex flex-col lg:flex-row justify-between items-start lg:items-end gap-4 mb-6; }
.greeting-section { @apply flex-1; }
.status-badge { @apply flex items-center gap-2 mb-2; }
.status-pulse { @apply relative flex h-2.5 w-2.5; }
.status-ping { @apply animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75; }
.status-dot { @apply relative inline-flex rounded-full h-2.5 w-2.5 bg-green-500 shadow-lg; box-shadow: 0 0 8px rgba(74, 222, 128, 0.5); }
.status-text { @apply text-blue-300/70 text-[10px] font-bold tracking-widest uppercase; }
.greeting-title { @apply text-5xl font-extrabold text-white tracking-tight; }
.greeting-name { @apply text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400; background-size: 200% auto; animation: gradient-x 3s ease infinite; }
@keyframes gradient-x { 0%, 100% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } }
.desktop-clock { @apply text-right; }
.clock-time { @apply text-6xl font-thin text-white tracking-tight drop-shadow-2xl flex items-baseline gap-2; }
.clock-period { @apply text-2xl font-medium text-gray-400; }
.clock-date { @apply text-white/40 font-medium text-xs uppercase tracking-widest mt-1; }
...
.desktop-feature-card { @apply relative overflow-hidden flex flex-col justify-between p-5 rounded-3xl shadow-xl border border-white/10 transition-all duration-500 cursor-pointer opacity-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-white; }
.desktop-feature-card:hover { @apply scale-105 shadow-2xl; }
.desktop-feature-card:active { @apply scale-95; }
.desktop-feature-large { @apply col-span-2 h-40; }
.desktop-feature-medium { @apply h-32; }
.feature-pattern { @apply absolute inset-0 opacity-10; background-image: url('https://www.transparenttextures.com/patterns/carbon-fibre.png'); }
.feature-shine { @apply absolute inset-0 bg-gradient-to-br from-white/[0.08] via-transparent to-transparent opacity-0 transition-opacity duration-500; }
.desktop-feature-card:hover .feature-shine { @apply opacity-100; }
.feature-shine-rotate { @apply absolute inset-0 -translate-x-full transition-transform duration-1000; background: linear-gradient(to right, transparent, rgba(255, 255, 255, 0.2), transparent); }
.desktop-feature-card:hover .feature-shine-rotate { @apply translate-x-full; }
.feature-badge { @apply absolute top-4 left-4 z-10 px-2.5 py-1 rounded-full text-[10px] font-bold text-white uppercase tracking-wider shadow-xl; background: rgba(255, 255, 255, 0.15); backdrop-filter: blur(20px); border: 1px solid rgba(255, 255, 255, 0.25); }
.feature-history-link { @apply absolute top-4 right-4 z-10 flex items-center gap-1 pl-2.5 pr-1.5 py-1 rounded-full border border-white/15 text-[10px] font-bold text-white uppercase tracking-widest transition-all duration-300 shadow-lg; background: rgba(0, 0, 0, 0.3); backdrop-filter: blur(20px); }
.feature-history-link:hover { background: rgba(0, 0, 0, 0.5); @apply scale-110; }
.feature-history-icon { @apply w-3 h-3 text-white/70; }
.feature-content { @apply relative z-0 flex flex-col justify-between h-full; }
.feature-icon { @apply w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shadow-xl transition-all duration-500; background: rgba(255, 255, 255, 0.2); backdrop-filter: blur(20px); }
.desktop-feature-card:hover .feature-icon { @apply scale-125 rotate-12; }
...
.id-card-content { @apply h-full p-6 flex flex-col justify-between relative overflow-hidden; background: linear-gradient(135deg, rgba(51, 65, 85, 1), rgba(30, 41, 59, 1), rgba(0, 0, 0, 1)); }
.id-card-gradient { @apply absolute inset-0; background: linear-gradient(135deg, rgba(59, 130, 246, 0.05), rgba(147, 51, 234, 0.05), transparent); animation: gradient-rotate 20s ease-in-out infinite; }
.id-card-header { @apply flex justify-between items-start relative z-10; }
.id-card-logo { @apply w-10 h-10 rounded-full flex items-center justify-center text-lg border border-white/20 shadow-xl; background: rgba(255, 255, 255, 0.15); backdrop-filter: blur(20px); }
...
.id-card-name { @apply text-2xl text-white font-black tracking-wide uppercase mb-1 truncate drop-shadow-lg; }
.id-card-badges { @apply flex items-center gap-2 flex-wrap; }
.id-card-badge { @apply px-2 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider shadow-lg border; }
.id-card-badge-primary { background: linear-gradient(to right, rgba(59, 130, 246, 0.25), rgba(37, 99, 235, 0.25)); @apply text-blue-300 border-blue-500/40; box-shadow: 0 4px 12px rgba(59, 130, 246, 0.2); }
.id-card-badge-success { background: linear-gradient(to right, rgba(34, 197, 94, 0.25), rgba(22, 163, 74, 0.25)); @apply text-green-300 border-green-500/40; box-shadow: 0 4px 12px rgba(34, 197, 94, 0.2); }

.sidebar-card { @apply block rounded-3xl overflow-hidden shadow-xl h-40 transition-all duration-500 opacity-0; }
.sidebar-card:hover { @apply scale-105; }
.sidebar-card-content { @apply h-full p-6 flex flex-col justify-between relative overflow-hidden; }
.sidebar-card-polls .sidebar-card-content { background: linear-gradient(135deg, rgba(202, 138, 4, 1), rgba(161, 98, 7, 1)); }
.sidebar-card-emergency .sidebar-card-content { background: linear-gradient(135deg, rgba(220, 38, 38, 1), rgba(185, 28, 28, 1)); }
.sidebar-card-pattern { @apply absolute inset-0 opacity-10; background-image: url('https://www.transparenttextures.com/patterns/carbon-fibre.png'); }
.sidebar-card-shine { @apply absolute inset-0 bg-gradient-to-br from-white/[0.08] via-transparent to-transparent opacity-0 transition-opacity duration-500; }
...
.sidebar-card-icon { @apply w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shadow-xl; background: rgba(255, 255, 255, 0.25); }
.sidebar-card-badge { @apply text-[10px] font-black px-2 py-1 rounded uppercase tracking-widest shadow-lg; background: rgba(255, 255, 255, 1); }
.sidebar-card-polls .sidebar-card-badge { @apply text-amber-600; }
.sidebar-emergency-badge { @apply animate-pulse text-red-600; background: rgba(255, 255, 255, 1); }
.sidebar-card-footer { @apply relative z-10; }
.sidebar-card-title { @apply font-black text-white text-2xl uppercase tracking-wide drop-shadow-lg; }
.sidebar-card-subtitle { @apply text-white/80 text-xs font-bold mt-1; }
.sidebar-emergency-pulse-1, .sidebar-emergency-pulse-2 { @apply absolute -top-2 -right-2 w-6 h-6 rounded-full animate-ping; }
.sidebar-emergency-pulse-1 { @apply bg-white/30; }
.sidebar-emergency-pulse-2 { @apply bg-white/20; animation-delay: 0.5s; }

.sidebar-map-card { @apply relative rounded-3xl p-1 h-80 shadow-2xl opacity-0; background: rgba(0, 0, 0, 0.3); backdrop-filter: blur(40px); border: 1px solid rgba(255, 255, 255, 0.10); }
.sidebar-map-badge { @apply absolute top-4 left-4 z-10 px-3 py-1 rounded-full text-xs font-bold text-white border border-white/15 shadow-xl; background: rgba(0, 0, 0, 0.7); backdrop-filter: blur(20px); }
.sidebar-map-wrapper { @apply w-full h-full rounded-[1.8rem] overflow-hidden opacity-90 transition-opacity duration-500; }
.sidebar-map-card:hover .sidebar-map-wrapper { @apply opacity-100; }

/* Animations */
.animate-fade-in { animation: fade-in 0.7s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
@keyframes fade-in { from { opacity: 0; transform: translateY(-20px); } to { opacity: 1; transform: translateY(0); } }
.animate-scale-in { animation: scale-in 0.7s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
@keyframes scale-in { from { opacity: 0; transform: scale(0.9); } to { opacity: 1; transform: scale(1); } }
.animate-slide-up { animation: slide-up 0.7s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
@keyframes slide-up { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
.animate-slide-right { animation: slide-right 0.7s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
@keyframes slide-right { from { opacity: 0; transform: translateX(-30px); } to { opacity: 1; transform: translateX(0); } }
.animate-scale-fade { animation: scale-fade 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
