...
// Enhanced sound with Web Audio API
        ...
        <div class="mobile-layout">
            ...
            <div class="stats-grid">
                <div v-for="(stat, index) in quickStats" :key="stat.label"
                    ...
                    :style="`animation-delay: ${100 + index * 80}ms; transform: translateY(${scrollY * 0.1}px)`">
...
@keyframes particle-float { 
    ...
    50% { transform: translate(80px, -100px); opacity: 0.8; } 
}
...
.skip-btn { 
    @apply absolute top-8 right-8 z-50 px-6 py-3 bg-white/5 backdrop-blur-xl border border-white/10 rounded-full text-white text-sm font-semibold hover:bg-white/10 transition-all duration-300 flex items-center gap-2 hover:scale-105 active:scale-95; 
...
.logo-subtitle { 
    @apply text-xl text-gray-400 font-light tracking-widest uppercase; 
}

/* Welcome Phase */
.welcome-badge { 
    @apply inline-flex items-center gap-3 px-6 py-3 rounded-full mb-10 bg-blue-500/10 border border-blue-500/30 backdrop-blur-xl; 
}

.welcome-pulse { @apply relative flex h-3 w-3; }

.pulse-ring { 
    @apply animate-ping absolute h-full w-full rounded-full bg-blue-400 opacity-75;
    will-change: transform, opacity;
}
...
.welcome-badge-text { 
    @apply text-sm font-bold text-blue-300 uppercase tracking-[0.3em]; 
}
...
.feature-subtitle { 
    @apply text-base font-bold uppercase tracking-[0.4em] mb-4; 
}

.feature-title { 
    @apply text-6xl md:text-7xl font-black text-white mb-6; 
}

.feature-desc { 
    @apply text-2xl text-gray-400 font-light max-w-3xl mx-auto mb-10; 
}
...
/* Stats Phase */
.stats-title { 
    @apply text-5xl md:text-6xl font-black text-white mb-12; 
}
...
.stat-glow { 
    @apply absolute -top-10 -right-10 w-32 h-32 opacity-40 blur-3xl rounded-full; 
}
...
.stat-label { 
    @apply text-sm text-gray-400 font-semibold uppercase tracking-wider; 
}
...
/* Toast */
.toast-notification { 
    @apply fixed top-20 right-4 z-[60] w-[90vw] sm:w-96 rounded-2xl shadow-2xl overflow-hidden flex items-center gap-3; 
...
.toast-icon-pulse { 
    @apply absolute inset-0 bg-white/20 rounded-xl animate-ping;
    will-change: transform, opacity;
}
...
.toast-text { 
    @apply text-sm font-bold text-white truncate drop-shadow-lg; 
}

.toast-close-btn { 
    @apply flex-shrink-0 p-2 hover:bg-white/20 rounded-lg transition-colors active:scale-90; 
}
...
.mobile-layout { 
    @apply lg:hidden w-full max-w-md mx-auto px-4 pb-8 pt-8; 
}
...
.time-widget-container:hover .time-value {
...
.date-badge { 
    @apply relative px-4 py-2 rounded-2xl border border-white/[0.12] shadow-lg overflow-hidden transition-transform duration-300; 
...
.date-badge-shine { 
    @apply absolute inset-0 bg-gradient-to-br from-white/10 via-transparent to-transparent opacity-0 transition-opacity duration-500; 
}

.date-badge:hover .date-badge-shine { @apply opacity-100; }

.date-badge-day { 
    @apply text-2xl font-bold text-white text-center leading-none mb-0.5; 
}
...
.stat-card { 
    @apply relative overflow-hidden rounded-[1.75rem] p-5 shadow-xl transition-all duration-500 border border-white/[0.10] opacity-0; 
    background: linear-gradient(135deg, rgba(28, 28, 30, 1), rgba(44, 44, 46, 1));
...
.stat-card-glow { 
    @apply absolute -top-10 -right-10 w-24 h-24 opacity-25 blur-2xl rounded-full transition-all duration-700;
    will-change: opacity, transform;
}

.stat-card:hover .stat-card-glow { 
    @apply opacity-40 scale-125; 
}

.stat-card-shine { 
    @apply absolute inset-0 bg-gradient-to-br from-white/[0.05] via-transparent to-transparent opacity-0 transition-opacity duration-500; 
}
...
.stat-icon-badge { 
    @apply w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg transition-all duration-500; 
...
.stat-value { 
    @apply text-3xl font-bold text-white mb-0.5 tracking-tight transition-transform duration-300;
    will-change: transform;
}
...
.stat-highlight { 
    @apply absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-white/[0.08] to-transparent transition-all duration-500; 
}

.stat-card:hover .stat-highlight { 
    @apply via-white/[0.15]; 
}
...
.search-glow { 
    @apply absolute -inset-0.5 rounded-[2rem] opacity-0 blur-2xl transition-all duration-700; 
...
.search-bar { 
    @apply relative bg-[#1c1c1e] rounded-[2rem] border border-white/[0.10] shadow-xl overflow-hidden transition-all duration-500;
...
.search-highlight { 
    @apply absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/[0.12] to-transparent; 
}

.search-icon-wrapper { 
    @apply absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center; 
}
...
.search-input { 
    @apply w-full px-5 py-4 pl-14 bg-transparent text-white placeholder-gray-500 text-[15px] focus:outline-none; 
}

.search-voice-btn { 
    @apply absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center rounded-xl transition-all duration-300 active:scale-90; 
...
.apps-view-all { 
    @apply text-xs font-semibold text-blue-400 hover:text-blue-300 active:scale-95 transition-all duration-300;
...
.app-item { 
    @apply flex flex-col items-center gap-2 rounded-3xl p-2 active:scale-90 transition-all duration-500 opacity-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500;
    will-change: transform, opacity;
}
...
.app-pulse-ring { 
    @apply absolute inset-0 bg-red-500 rounded-[1.3rem] animate-ping opacity-40;
    will-change: transform, opacity;
}

.app-glow { 
    @apply absolute -inset-1 rounded-[1.4rem] opacity-0 blur-lg transition-all duration-500;
    will-change: opacity;
}
...
.app-icon-wrapper { 
    @apply relative w-full h-full rounded-[1.3rem] shadow-xl flex items-center justify-center transition-all duration-500 overflow-hidden;
    will-change: transform;
}
...
.app-icon-shine-rotate { 
    @apply absolute inset-0 rounded-[1.3rem] -translate-x-full transition-transform duration-1000; 
...
.app-icon-emoji { 
    @apply relative text-[32px] filter drop-shadow-2xl transition-transform duration-300;
    will-change: transform;
}
...
.app-name { 
    @apply text-[11px] font-semibold text-gray-300 text-center leading-tight w-full px-1; 
}
...
.featured-dot { 
    @apply w-1.5 h-1.5 rounded-full animate-pulse shadow-lg;
    will-change: opacity;
}
...
.featured-card { 
    @apply relative overflow-hidden rounded-[1.75rem] p-4 border border-white/[0.10] shadow-xl transition-all duration-500 active:scale-[0.97] opacity-0; 
    background: linear-gradient(135deg, rgba(28, 28, 30, 1), rgba(44, 44, 46, 1));
    will-change: transform, opacity, box-shadow;
}
...
.featured-card-icon { 
    @apply mb-3 w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg transition-all duration-500;
    will-change: transform;
}
...
.featured-card-arrow { 
    @apply absolute bottom-4 right-4 w-6 h-6 rounded-full flex items-center justify-center transition-all duration-300 z-10; 
    background: rgba(255, 255, 255, 0.10);
    will-change: transform, background;
}
...
/* Map */
.map-preview-card { 
    @apply relative overflow-hidden rounded-[1.75rem] border border-white/[0.10] shadow-xl opacity-0; 
    background: linear-gradient(135deg, rgba(28, 28, 30, 1), rgba(44, 44, 46, 1));
    will-change: opacity, transform;
}
...
.map-badge { 
    @apply absolute top-4 left-4 z-10 flex items-center gap-2 px-3 py-2 rounded-full border border-white/[0.10] shadow-xl; 
...
.map-info-bar { 
    @apply absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 via-black/50 to-transparent; 
    backdrop-filter: blur(20px); 
}
...
.map-info-title { 
    @apply text-sm font-bold text-white drop-shadow-lg; 
}
...
.emergency-glow { 
    @apply absolute inset-0 bg-red-500 rounded-full blur-2xl opacity-60 transition-opacity duration-500;
    will-change: opacity;
}
...
.emergency-btn { 
    @apply relative w-16 h-16 rounded-full flex items-center justify-center active:scale-90 transition-all duration-300 border-4 border-black/30 overflow-hidden; 
...
.emergency-btn-shine-rotate { 
    @apply absolute inset-0 rounded-full -translate-x-full transition-transform duration-1000; 
...
.emergency-icon { 
    @apply relative text-3xl filter drop-shadow-2xl z-10; 
}

.emergency-label { 
    @apply absolute -bottom-7 left-1/2 -translate-x-1/2 whitespace-nowrap opacity-0 transition-opacity duration-300;
    will-change: opacity;
}
...
.clock-time { 
    @apply text-6xl font-thin text-white tracking-tight drop-shadow-2xl flex items-baseline gap-2; 
}
...
.desktop-feature-card { 
    @apply relative overflow-hidden flex flex-col justify-between p-5 rounded-3xl shadow-xl border border-white/10 transition-all duration-500 cursor-pointer opacity-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-white;
    will-change: transform, opacity, box-shadow;
}
...
.feature-pattern { 
    ...
    background-image: url('data:image/svg+xml,%3Csvg width="60" height="60" xmlns="http://www.w3.org/2000/svg"%3E%3Cpath d="M0 0h60v60H0z" fill="none"/%3E%3Cpath d="M30 0v60M0 30h60" stroke="%23fff" stroke-width=".5" opacity=".1"/%3E%3C/svg%3E');
}

.feature-shine { 
    @apply absolute inset-0 bg-gradient-to-br from-white/[0.08] via-transparent to-transparent opacity-0 transition-opacity duration-500; 
}
...
.feature-badge { 
    @apply absolute top-4 left-4 z-10 px-2.5 py-1 rounded-full text-[10px] font-bold text-white uppercase tracking-wider shadow-xl; 
    background: rgba(255, 255, 255, 0.15); 
    backdrop-filter: blur(20px); 
...
.feature-history-link { 
    @apply absolute top-4 right-4 z-10 flex items-center gap-1 pl-2.5 pr-1.5 py-1 rounded-full border border-white/15 text-[10px] font-bold text-white uppercase tracking-widest transition-all duration-300 shadow-lg; 
...
.feature-icon { 
    @apply w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shadow-xl transition-all duration-500; 
...
.feature-title { 
    @apply font-black text-white text-xl leading-tight drop-shadow-lg; 
}

.desktop-sidebar { @apply lg:col-span-4 space-y-6; }

.id-card { 
    @apply aspect-[1.6/1] rounded-3xl overflow-hidden shadow-2xl opacity-0;
    will-change: opacity, transform;
}
...
.id-card-logo { 
    @apply w-10 h-10 rounded-full flex items-center justify-center text-lg border border-white/20 shadow-xl; 
    background: rgba(255, 255, 255, 0.15); 
    backdrop-filter: blur(20px); 
}
...
.sidebar-card { 
    @apply block rounded-3xl overflow-hidden shadow-xl h-40 transition-all duration-500 opacity-0;
    will-change: transform, opacity;
}
...
.sidebar-card-icon { 
    @apply w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shadow-xl; 
    background: rgba(255, 255, 255, 0.25); 
}
...
.sidebar-map-badge { 
    @apply absolute top-4 left-4 z-10 px-3 py-1 rounded-full text-xs font-bold text-white border border-white/15 shadow-xl; 
    background: rgba(0, 0, 0, 0.7); 
    backdrop-filter: blur(20px); 
}

.sidebar-map-wrapper { 
    @apply w-full h-full rounded-[1.8rem] overflow-hidden opacity-90 transition-opacity duration-500;
    will-change: opacity;
}
...
@media (max-width: 640px) {
    ...
    .apps-grid { @apply gap-3; }
    .app-icon-emoji { @apply text-[28px]; }
}

@media (min-width: 641px) and (max-width: 1023px) {
    .mobile-layout { @apply max-w-2xl px-6; }
    .apps-grid { @apply grid-cols-6 gap-4; }
    .stats-grid { @apply gap-4; }
}

@media (min-width: 1024px) and (max-width: 1279px) {
    .desktop-features-grid { @apply gap-3; }
    .desktop-feature-card { @apply p-4; }
    .feature-title { @apply text-lg; }
}

@media (min-width: 1280px) {
    .desktop-layout { @apply max-w-[90rem]; }
}
