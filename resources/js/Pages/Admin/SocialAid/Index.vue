<script>
// ════════════════════════════════════════════════════════════════
// FILE: resources/js/Pages/Admin/SocialAid/Index.vue
// ════════════════════════════════════════════════════════════════
</script>
...
const statusCfg = {
    ...
    released:     { label: 'Released',     badge: 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30' },
};
const getStatus = (s) => statusCfg[s] ?? { label: s, badge: 'bg-slate-500/20 text-slate-400 border-slate-500/30' };

const programIcon = (type) => ({ 'Student Assistance':'🎓', 'Senior Citizen':'👴', 'PWD Assistance':'♿', 'Solo Parent':'👩‍👧', 'Calamity Aid':'🆘' }[type] ?? '🤝');
...
<template>
    <Head title="Social Aid Applications" />
    <AuthenticatedLayout>
        <div class="min-h-screen bg-slate-950 text-white font-sans">
            <div class="fixed inset-0 pointer-events-none z-0">
                <div class="absolute inset-0 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950"></div>
                <div class="absolute top-0 right-1/4 w-[500px] h-[400px] bg-rose-500/6 rounded-full blur-[100px]"></div>
            </div>
            ...
            <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <div class="flex items-center gap-2 mb-1 text-sm text-slate-500">
                            ...
                            <span>/</span><span class="text-slate-400">Social Aid</span>
                        </div>
                        <h1 class="text-2xl font-bold flex items-center gap-2">🤝 Social Aid Applications
                            <span class="text-sm px-2 py-0.5 bg-rose-500/20 border border-rose-500/30 text-rose-400 rounded-full font-semibold">{{ total }} total</span>
                        </h1>
                    </div>
                ...
                <div class="relative">
                    ...
                    <input v-model="search" type="text" placeholder="Search applicant, program, tracking code..."
                        class="w-full pl-11 pr-4 py-3 bg-slate-900/60 backdrop-blur border border-slate-700/50 rounded-xl text-white placeholder-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-rose-500/40 transition-all"/>
                </div>
                ...
                <div v-if="filtered.length === 0" class="text-center py-20 bg-slate-900/40 backdrop-blur border border-slate-800/50 rounded-2xl">
                    ...
                    <p class="text-slate-400 font-semibold">No applications found</p>
                    <p class="text-slate-600 text-sm mt-1">Applications will appear here once submitted by residents</p>
                </div>
                ...
                <div v-else class="bg-slate-900/60 backdrop-blur border border-slate-800/50 rounded-2xl overflow-hidden">
                    <div class="hidden lg:grid grid-cols-12 gap-4 px-6 py-3 border-b border-slate-800/50 bg-slate-900/80">
                        ...
                        <div class="col-span-3 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Program</div>
                    ...
                    <div class="divide-y divide-slate-800/50">
                        <div v-for="app in filtered" :key="app.id" class="group hover:bg-white/[0.02] transition-colors">
                            <div class="hidden lg:grid grid-cols-12 gap-4 px-6 py-4 items-center">
                                <div class="col-span-2 font-mono text-xs text-rose-400 font-bold">{{ app.tracking_code }}</div>
                                <div class="col-span-3 flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-rose-500 to-pink-600 flex items-center justify-center text-sm font-bold flex-shrink-0">
                                        {{ app.user?.name?.charAt(0) ?? '?' }}
                                    </div>
                                    <div class="min-w-0">
                                        <p class="text-sm font-semibold text-white truncate">{{ app.user?.name ?? 'Unknown' }}</p>
                                        <p class="text-xs text-slate-500 truncate">{{ app.user?.email }}</p>
                                    </div>
                                </div>
                                <div class="col-span-3 flex items-center gap-2">
                                    <span class="text-xl">{{ programIcon(app.program_type) }}</span>
                                    <div>
                                        <p class="text-sm text-slate-200">{{ app.program_type }}</p>
                                        <p class="text-xs text-slate-500" v-if="app.release_date">Release: {{ fmt(app.release_date) }}</p>
                                    </div>
                                </div>
                                <div class="col-span-2">
                                    <span :class="['inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold border', getStatus(app.status).badge]">
                                        {{ getStatus(app.status).label }}
                                    </span>
                                </div>
                                <div class="col-span-1 text-xs text-slate-500 font-mono">{{ fmt(app.created_at) }}</div>
                                <div class="col-span-1 flex justify-end">
                                    <Link :href="route('admin.social-aid.show', app.id)"
                                        class="px-3 py-1.5 bg-rose-600/20 hover:bg-rose-600/40 border border-rose-500/30 rounded-lg text-rose-400 text-xs font-bold transition-all opacity-0 group-hover:opacity-100">
                                        Review →
                                    </Link>
                                </div>
                            </div>
                            ...
                            <div class="lg:hidden p-4 flex items-start justify-between gap-3">
                                <div class="flex items-center gap-3 flex-1 min-w-0">
                                    <div class="text-2xl">{{ programIcon(app.program_type) }}</div>
                                    <div class="min-w-0">
                                        <p class="text-sm font-bold text-white">{{ app.user?.name }}</p>
                                        <p class="text-xs text-slate-400">{{ app.program_type }}</p>
                                        <p class="text-xs font-mono text-rose-400 mt-0.5">{{ app.tracking_code }}</p>
                                    </div>
                                </div>
                                <div class="flex flex-col items-end gap-2">
                                    ...
                                    <Link :href="route('admin.social-aid.show', app.id)" class="px-3 py-1.5 bg-rose-600/20 border border-rose-500/30 rounded-lg text-rose-400 text-xs font-bold">Review →</Link>
                                </div>
                            </div>
                        </div>
                    </div>
