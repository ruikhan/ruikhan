...
// --- 1. SAFE PROPS DEFINITION ---
// We add default values (default: () => []) so the page never crashes
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mb-8 bg-slate-800/50 backdrop-blur-md border border-slate-700 p-4 rounded-2xl shadow-xl">
                ...
                <div class="flex gap-6 mt-4 md:mt-0 overflow-x-auto">
                    <div class="text-right">
                        ...
                        <p class="text-green-400 font-mono font-bold">{{ formatMoney(stats?.revenue) }}</p>
                    </div>
                    <div class="text-right border-l border-slate-700 pl-6">
                        <p class="text-[10px] text-slate-400 uppercase font-bold">New Citizens</p>
                        <p class="text-blue-400 font-mono font-bold">+{{ stats?.citizens || 0 }}</p>
                    </div>
                    <div class="text-right border-l border-slate-700 pl-6">
                        <p class="text-[10px] text-slate-400 uppercase font-bold">Pending Actions</p>
                        <p class="text-yellow-400 font-mono font-bold">{{ pendingTasks?.length || 0 }}</p>
                    </div>
                </div>
            </div>
                        ...
                        <div class="divide-y divide-slate-700/50">
                            <div v-for="task in pendingTasks" :key="task.id" 
                                ...
                                <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <Link :href="route('admin.documents.show', task.id)" class="px-3 py-1 bg-green-600 hover:bg-green-500 text-white text-xs rounded font-bold">View</Link>
                                </div>
                            </div>
                            
                            <div v-if="pendingTasks?.length === 0" class="p-8 text-center text-slate-500 text-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="bg-slate-800/80 backdrop-blur-xl border border-slate-700 rounded-2xl p-6 shadow-2xl">
                             ...
                             <div class="space-y-4">
                                ...
                                <div v-if="departmentLoad?.length === 0" class="text-center text-slate-500 text-xs">
                        ...
                        <div class="bg-gradient-to-br from-indigo-900/50 to-slate-900/50 border border-indigo-500/30 rounded-2xl p-6 shadow-2xl">
                             ...
                             <div class="grid grid-cols-2 gap-3">
                                <Link :href="route('admin.documents.index')" class="p-3 bg-slate-800 hover:bg-indigo-600 transition rounded-lg text-center border border-slate-600 group">
                                    <div class="text-xl mb-1 group-hover:scale-110 transition">📄</div>
                                    <span class="text-xs text-white font-bold">Docs</span>
                                </Link>
                                
                                <div class="p-3 bg-slate-800 hover:bg-red-600 transition rounded-lg text-center border border-slate-600 group cursor-pointer">
                                    <div class="text-xl mb-1 group-hover:scale-110 transition">🚨</div>
                                    <span class="text-xs text-white font-bold">Dispatch</span>
                                </div>
                                <div class="p-3 bg-slate-800 hover:bg-green-600 transition rounded-lg text-center border border-slate-600 group cursor-pointer">
                                    <div class="text-xl mb-1 group-hover:scale-110 transition">📈</div>
                                    <span class="text-xs text-white font-bold">Prices</span>
                                </div>
                                <div class="p-3 bg-slate-800 hover:bg-purple-600 transition rounded-lg text-center border border-slate-600 group cursor-pointer">
                                    <div class="text-xl mb-1 group-hover:scale-110 transition">💼</div>
                                    <span class="text-xs text-white font-bold">Jobs</span>
                                </div>
                             </div>
                          </div>
                    </div>
