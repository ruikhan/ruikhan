...
const showingMobileMenu = ref(false); // <--- Added for Mobile
const page = usePage();
...
router.on('start', () => { 
    ...
    showingMobileMenu.value = false; // Close mobile menu on navigate
});
...
<template>
    <div class="min-h-screen bg-[#000000] text-slate-100 font-sans relative selection:bg-blue-500/30 selection:text-blue-200">
        ...
        <nav class="fixed top-6 left-0 right-0 z-40 flex justify-center px-4 transition-all duration-700 cubic-bezier(0.16, 1, 0.3, 1)"
            ...
            <div class="group relative flex items-center justify-between gap-6 px-3 py-2.5 bg-[#121212]/70 backdrop-blur-2xl border border-white/10 rounded-full shadow-[0_20px_40px_-10px_rgba(0,0,0,0.5)] ring-1 ring-white/5 transition-all duration-300 hover:scale-[1.01] hover:bg-[#121212]/80 w-full max-w-fit md:w-auto">
                
                <div class="absolute inset-x-8 top-0 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent"></div>

                <Link :href="route('dashboard')" class="pl-4 pr-2 flex items-center gap-3 shrink-0">
                ...
                <div class="hidden md:flex items-center gap-1">
                    ...
                    <template v-else>
                        ...
                        <Link :href="route('services.index')" :class="['nav-pill', route().current('services.*') ? 'active' : '']">
                            📄 My Docs
                        </Link>
                        ...
                        <Link :href="route('emergency.index')" :class="['nav-pill text-red-400 hover:text-red-300', route().current('emergency.*') ? 'active-danger' : '']">
                            🚨 Emergency
                        </Link>
                    </template>
                </div>
                    ...
                    <Dropdown align="right" width="48">
                        ...
                        <template #content>
                            <div class="p-1 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-xl backdrop-blur-xl">
                                ...
                                <DropdownLink :href="route('proposal')" class="rounded-lg text-gray-300 hover:text-white hover:bg-white/10">
                                    📖 Barangay Proposal
                                </DropdownLink>
                                <DropdownLink :href="route('simulation')" class="rounded-lg text-gray-300 hover:text-white hover:bg-white/10">
                                    📖 System Handbook
                                </DropdownLink>
                                <DropdownLink :href="route('training')" class="rounded-lg text-gray-300 hover:text-white hover:bg-white/10">
                                    🖧 Training Assessment
                                </DropdownLink>
                                <DropdownLink :href="route('logout')" method="post" as="button" class="rounded-lg text-red-400 hover:bg-red-500/10 w-full text-left">
                                    🚪 Log Out
                                </DropdownLink>
                            </div>
                        </template>
            ...
            >
                <div v-show="showingMobileMenu" class="absolute top-20 left-4 right-4 bg-[#1a1a1a]/95 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-2xl md:hidden z-50 flex flex-col gap-2">
                    <Link :href="route('dashboard')" class="mobile-nav-item" :class="{'bg-white/10 text-white': route().current('dashboard')}">
                        🏠 Dashboard
                    </Link>
                    
                    <template v-if="user.role === 'admin'">
                        <Link :href="route('admin.analytics')" class="mobile-nav-item">📊 Command Center</Link>
                        <Link :href="route('admin.documents.index')" class="mobile-nav-item">✅ Approvals</Link>
                    </template>
                    
                    <template v-else>
                         <Link :href="route('services.index')" class="mobile-nav-item">📄 My Documents</Link>
                         <Link :href="route('emergency.index')" class="mobile-nav-item text-red-400">🚨 Emergency</Link>
                         <Link v-if="isBusinessOwner" :href="route('business.dashboard')" class="mobile-nav-item text-blue-300">📊 My Business</Link>
                         <Link v-else :href="route('business.register')" class="mobile-nav-item text-green-300">🚀 Start Business</Link>
                    </template>
                </div>
            </transition>
        ...
        <Transition name="shutter">
            <div v-if="isLoading" class="fixed inset-0 z-[100] flex flex-col items-center justify-center">
                ...
                <div class="relative z-20 flex flex-col items-center animate-content-reveal">
                    ...
                    <img :src="logoUrl" 
                         class="h-28 w-auto relative drop-shadow-[0_10px_40px_rgba(59,130,246,0.4)] animate-logo-float" 
                         alt="E-Pili Loading" />
...
/* NAV PILLS */
.nav-pill {
    @apply px-4 py-1.5 rounded-full text-xs font-medium text-white/50 hover:text-white hover:bg-white/5 transition-all duration-300 whitespace-nowrap;
}
.nav-pill.active {
...
/* Mobile Nav Items */
.mobile-nav-item {
    @apply block px-4 py-3 rounded-xl text-sm font-medium text-gray-400 hover:text-white hover:bg-white/5 transition-colors;
}
...
.shutter-enter-from { opacity: 0; transform: scale(1.03); }
.shutter-enter-to { opacity: 1; transform: scale(1); }
.shutter-leave-from { opacity: 1; transform: scale(1); }
.shutter-leave-to { opacity: 0; transform: scale(0.98); }
...
@keyframes backdrop-fade-in {
    ...
    100% { opacity: 1; backdrop-filter: blur(60px); }
}
...
@keyframes logo-float {
    0% { transform: translateY(0px) scale(0.95); opacity: 0; }
...
@keyframes dot-wave {
    0%, 60%, 100% { transform: translateY(0px) scale(1); opacity: 0.5; }
