        ...
        <nav class="fixed top-6 left-0 right-0 z-40 flex justify-center px-4 transition-all duration-700 cubic-bezier(0.16, 1, 0.3, 1)"
            ...
            <div class="group relative flex items-center gap-6 px-3 py-2.5 bg-[#121212]/70 backdrop-blur-2xl border border-white/10 rounded-full shadow-[0_20px_40px_-10px_rgba(0,0,0,0.5)] ring-1 ring-white/5 transition-all duration-300 hover:scale-[1.01] hover:bg-[#121212]/80">
                ...
                <Link :href="route('dashboard')" class="pl-4 pr-2 flex items-center gap-3">
                    <img :src="logoUrl" alt="Logo" class="h-16 w-auto drop-shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
                </Link>
                ...
                <div class="hidden md:flex items-center gap-1">
                    ...
                    <template v-if="user.role === 'admin'">
                        <Link :href="route('admin.analytics')" :class="['nav-pill', route().current('admin.analytics') ? 'active-danger' : '']">Command</Link>
                        <Link :href="route('admin.documents.index')" :class="['nav-pill', route().current('admin.documents.*') ? 'active' : '']">Approvals</Link>
                    </template>
                    <template v-else>
                        <Link :href="route('documents.index')" :class="['nav-pill', route().current('documents.*') ? 'active' : '']">Docs</Link>
                        <Link :href="route('market.index')" :class="['nav-pill', route().current('market.*') ? 'active' : '']">Market</Link>
                        <Link :href="route('establishments.index')" :class="['nav-pill', route().current('establishments.*') ? 'active' : '']">Places</Link>
                    </template>
                </div>
                    ...
                    <Dropdown align="right" width="48">
                        ...
                        <template #content>
                            <div class="p-1 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-xl backdrop-blur-xl">
                                <DropdownLink :href="route('profile.edit')" class="rounded-lg text-gray-300 hover:text-white hover:bg-white/10">Profile</DropdownLink>
                                <DropdownLink :href="route('logout')" method="post" as="button" class="rounded-lg text-red-400 hover:bg-red-500/10">Log Out</DropdownLink>
                            </div>
                        </template>
                    </Dropdown>
                </div>
